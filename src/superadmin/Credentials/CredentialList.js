...
const CredentialList = () => {
  ...
  const getCredentialListings = () => {
    axios
      .get(`${API_URL}/getSUCredentialList`, {
  ...
  const deleteCredentials = (credentialId) => {
    ...
    axios
      .post(`${API_URL}/deleteSUCredentialsById/?_method=DELETE`, data, {
        headers: { Authorization: token },
  ...
  const buttonTemplate = (responseData) => (
    <div style={{ display: "flex" }}>
      ...
      <Link
        to={`/superadmin/updatecredentials?${responseData.credentialId}`}
        onClick={() => {
          localStorage.setItem("credentialId", responseData.credentialId);
        }}
      >
