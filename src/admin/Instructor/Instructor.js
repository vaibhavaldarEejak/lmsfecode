...
const Instructor = () => {
  ...
  const getUserListing = async () => {
    try {
      const res = await getApiCall("getInstructorUserList");
      setRData(res);
    } catch (err) {}
  };

  const getInstructorListing = async () => {
    try {
      const res = await getApiCall("getInstructorList");
      setResponseData(res);
    } catch (err) {}
  };
  ...
  const adduserInstructor = async (data) => {
    try {
      const res = await postApiCall("userToInstructor", data);
      toast.current.show({
        severity: "success",
        summary: "Successful",
        detail: "User Successfully Assigned as Instructor",
        life: 3000,
      });
      getInstructorListing();
    } catch (err) {
                  ...
                  <CTabPane className="p-3 preview" visible>
                    <div className="card-header border-0 d-flex justify-content-between">
                      ...
                      >
                        ...
                        {/* Add user to Instructor list Modal  */}
                        <CModalBody>
                          <DataTable
                            value={rData}
                            removableSort
                          ...
                          body={
                            ...
                            //   <div>dadwqqwe</div>
                            // ) : (
                            <div>asdadasdasdasdadasd</div>
                            // )
                            // <>{rowData.groupnsAssignedNames.join(",")}</>
                            // )
                          }
                          sortable
                        ></Column> */}
