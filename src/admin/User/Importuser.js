...
const Importuser = () => {
  const token = "Bearer " + localStorage.getItem("ApiToken"),
    ...
    toast = useRef(null),
    [excelData, setExcelData] = useState([]),
  ...
  const downloadXLS = () => {
    ...
    link.download = `BulkUserImportExcel.xlsx`;
    link.href = `${process.env.PUBLIC_URL}/bulkImport/BulkUserImportExcel.xlsx`;
    link.click();
  };
  ...
  const handleSave = (e) => {
    ...
    if (!importFile) {
      toast.current.show({
        severity: "error",
        summary: "Error",
        detail: "The user import file field is required.",
        life: 3000,
      });
    }
    formData.append("userImportFile", importFile);
    setLoading1(true);
    axios
      .post(
        `${API_URL}/userImport`,
        {
          userImportFile: importFile,
        },
        {
          headers: {
            Authorization: token,
      ...
      .catch((error) => {
        ...
        if (
          ...
          if (Array.isArray(errors)) {
            errorMessage = errors
              .map((error) => {
                const { rowNumber, firstName, lastName, email } = error;
                const errorMessages = [];

                if (firstName) {
                  errorMessages.push(
                    `Row ${rowNumber}: First Name - ${firstName}`
                  );
                }
                if (lastName) {
                  errorMessages.push(
                    `Row ${rowNumber}: Last Name - ${lastName}`
                  );
                }
                if (email) {
                  errorMessages.push(`Row ${rowNumber}: Email - ${email}`);
                }
      ...
      <form>
        <CCol xs={12}>
          <CCard className="mb-4">
            <CCardHeader>
              <strong>Import Bulk Users</strong>
            </CCardHeader>
            <CCardBody
                            ...
                            <tbody>
                              {excelData.map((getdata, index) => (
                                <tr key={index}>
                                  ...
                                  <td>{getdata.jobtitle} </td>
                                  <td>{getdata.divisions} </td>
                                  <td>{getdata.area} </td>
                                  <td>{getdata.location} </td>
                                  <td>{getdata.barcode} </td>
