...
const manageCertificateListingWidget = () => {
  ...
  const getCertificateListing = async () => {
    try {
      const response = await getApiCall("getOrgCertificateList/?suAssigned=1");
  ...
  const dataReset = async (id) => {
    ...
    try {
      const response = await postApiCall("resetOrganizationNotification", data);

      toast.current.show({
        severity: "success",
        summary: "Successful",
        detail: "Certificate Reset Successfully",
        life: 3000,
      });
      ...
      toast.current.show({
        severity: "error",
        summary: "Error",
        detail: "Error Resetting Certificate",
        life: 3000,
      });
    }
  };
  ...
  const buttonTemplate = (responseData) => (
    <div style={{ display: "flex" }}>
      <div
        to={`/admin/certificatelist/viewcertificate`}
  ...
  const resetProductDialogFooter = () => (
    <React.Fragment>
      ...
      <Button
        ...
        icon="pi pi-check"
        severity="info"
        onClick={() => dataReset(selectedProduct.certificateId)}
      />
    </React.Fragment>
  );
                      ...
                      <CNavItem>
                        ...
                        >
                          <CNavLink active style={{ cursor: "pointer" }}>
                            ...
                            <strong>Assigned Certificates</strong>
                          </CNavLink>
                        </Link>
                      </CNavItem>
                      <CNavItem>
