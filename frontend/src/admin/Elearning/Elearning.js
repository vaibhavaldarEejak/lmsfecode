...
import Player from "../CourseCatalog/Player";
...
const Elearning = () => {
  ...
  const getElearningList = async () => {
    try {
      const response = await getApiCall("getCourseCatalogList?trainingType=1");
  ...
  const buttonTemplate = (responseData) => (
    <div style={{ display: "flex", justifyContent: "center" }}>
      <Link
        ...
        to={`/admin/coursecataloglist/viewcoursecatalog?elearning`}
      >
        <CImage
