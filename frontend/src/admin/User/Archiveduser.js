...
const Archiveduser = () => {
  ...
  const getArchiveUserListing = async () => {
    try {
      const res = await getApiCall(
        "getUserList?userType=archivedUser&sort=firstName&order=ASC"
      );
      setResponseData(res);
      setLoading(true);
    } catch (err) {}
  ...
  const activeuser = async (userId) => {
    ...
    try {
      const res = await postApiCall("activeUser", data);
