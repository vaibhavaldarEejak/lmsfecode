...
const Inactiveuser = () => {
  ...
  useEffect(() => {
    if (token !== "Bearer null") {
      getInactiveUserListing();
      initFilters();
    }
  }, []);

  const getInactiveUserListing = async () => {
    try {
      const res = await getApiCall("getUserList/?userType=inactivatedUser");
      setResponseData(res);
      setLoading(true);
    } catch (err) {
      setLoading(true);
  ...
  const Deleteuser = async (userId) => {
    ...
    try {
      ...
      toast.current.show({
        severity: "success",
        summary: "Successful",
        detail: "Inactive user Deleted Successfully!",
        life: 3000,
      });
    } catch (error) {
      toast.current.show({
