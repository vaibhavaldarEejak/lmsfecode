...
const Manualgroups = () => {
  ...
  const addgroupuser = async (data) => {
    try {
      ...
      setTimeout(() => {
        window.location.href = "/admin/manualgroup";
      }, 2000);
    } catch (error) {
      toast.current.show({
        severity: "error",
  ...
  const getGroupListing = async () => {
    console.log("getAPI");
    try {
      const response = await getApiCall("getOrgGroupList?isAuto=0");
      // Update the usersAssigned value for each group
      const updatedGroupData = response.map((group) => {
                ...
                <CCardBody>
                  ...
                  {!loader ? (
                    ...
                    >
                      ...
                      ></Column>
                      <Column header="Action" body={statusShow}></Column>
                    </DataTable>
                  )}
                  <Dialog
