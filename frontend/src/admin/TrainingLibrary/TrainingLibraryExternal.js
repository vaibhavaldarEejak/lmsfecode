...
const TrainingLibraryExternal = () => {
  ...
  useEffect(() => {
    if (token !== "Bearer null") {
      ...
      getTrainingLibraryExternalList();
      initFilters();
      getCategoryList();
    }
  }, []);
  ...
  const getTrainingLibraryExternalList = async () => {
    const res = await getApiCall("getCourseCatalogList");
    if (res) {
      const filteredData = res.filter(
        (items) => items.trainingLibraryId != null
      );
      ...
      if (!hasNavigated) {
        navigate("/admin/trainingLibraryExternal?All");
  ...
  const dataReset = async (id) => {
    ...
    try {
      const response = await postApiCall("courseCatalogReset", data);
  ...
  const buttonTemplate = (responseData) => (
    <div style={{ display: "flex" }}>
      <Link
        to={`/admin/coursecataloglist/viewcoursecatalog?trainingLibraryExternal`}
        onClick={() => {
            ...
            <div>
              <CCardHeader className="p-0">
                <CNav variant="tabs">
                  <CNavItem>
                    <Link
                      to={"/admin/trainingLibraryExternal?All"}
                  ...
                  <CNavItem>
                    <Link
                      to={"/admin/trainingLibraryExternal?eLearning"}
                      style={{ textDecoration: "none" }}
                    >
                      <CNavLink
                  ...
                  <CNavItem>
                    <Link
                      to={"/admin/trainingLibraryExternal?Classroom"}
                      style={{ textDecoration: "none" }}
                    >
                      <CNavLink
                  ...
                  <CNavItem>
                    <Link
                      to={"/admin/trainingLibraryExternal?Assessments"}
                      style={{ textDecoration: "none" }}
                    >
                      <CNavLink
                  ...
                  <CNavItem>
                    <Link
                      to={"/admin/trainingLibraryExternal?Archived"}
                      style={{ textDecoration: "none" }}
                    >
                      <CNavLink
...
export default TrainingLibraryExternal;
