...
const AddAssessmentLibraryPage = (questionDto) => {
  ...
  const validateForm = (e) => {
    ...
    if (!form.trainingCode) {
      messages.trainingCode = "training Code is required";
      isValid = false;
    } else {
  ...
  const addCouseButton = (e) => {
    if (validateForm4()) {
      ...
      // formdata.append("trainingCode", parseInt(form.trainingId1))
      formdata.append("trainingCode", parseInt(form.trainingCode));
  ...
  const updateButton = (e) => {
    ...
    if (validateForm4()) {
      ...
      updateFormData.append("trainingCode", parseInt(form.trainingCode));
  ...
  useMemo(() => {
    if (assessmentDetails) {
      setForm({
        ...
        trainingCode: assessmentDetails.trainingCode,
  ...
  const getTrainingCode = async () => {
    try {
      const res = await getApiCall("getCourseLibraryList",form);
      setForm(res.trainingCode);
    } catch (err) {
      console.log("err", err);
    }
  };
  // Call the function of getTrainingCode.
  useEffect(() => {
    getTrainingCode();
  }, []);
                          ...
                          <div className="col-sm-7">
                            <CFormInput
                              ...
                              onChange={(f) => {
                                setForm({
                                  ...form,
                                  trainingCode: f.target.value,
                                });
                              }}
                            ></CFormInput>
                            {showValidationMessages &&
                              validationMessages.trainingCode && (
                                <div
                                  className="fw-bolder"
                                  style={{ color: "red" }}
                                >
